...
function step2(){
	...
	for (s = 0; s < tableIndex; s++){ //dont use n
		//alert ("s="+s);
		index = index+1;
		...
		if (staName=="na" || isSetSTA==false){
			...
			if (staMadeCount==1){
				...
				allSeqs = document.getElementById(inputField).value;
			} else {
				allSTANames = allSTANames + "~" + GetStaCH(staName.toUpperCase());
				allSeqs = allSeqs + "~" + document.getElementById(inputField).value;
			}
...
function step4(){
	...
	for (m = 0; m < allMdls.Count; m++){
		...
		if (mdlName.indexOf(moldNo)==0){
			//alert ("same moldNo");
			if (allMdls.Item(m).GetParam("SRH_FOR_WRITE")!=null){
				//alert ("got SRH_FOR_WRITE");
