...
function replaceBoltsInAssembly ()
{
 ...
 \*--------------------------------------------------------------------*/  
  var bolt = session.GetModel ("BOLT", pfcCreate ("pfcModelType").MDL_PART); //model name = "BOLT"
  
  var row = bolt.GetRow (newInstance);    //pfcFamilyMember.GetRow()
  
  var newBolt = row.CreateInstance();  //pfcFamilyTableRow.CreateInstance()
